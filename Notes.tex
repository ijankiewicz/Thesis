...
The input bias current is a DC current required by the Op Amp's inputs to establish proper biasing of input stage transistors \cite{companion}. It ranges from a few microamps to a few picoamps. This current is going to flow through the input protection resistor, generating a voltage drop proportional to mentioned current value multiplied by the series resistance.
\par

Another unwanted, but also unavoidable error source is reverse leakage current of clamping diodes. 
This voltage drop is going to generate the output error as in the Equation \ref{bias}.

\begin{equation}
    V_{error} = (R_{ser} \times (I_{bias} + 2 \times I_{leak})) \times Gain
    \label{bias}
\end{equation}

The outline of the AC channel is presented in the Figure \ref{fig:input1}.
